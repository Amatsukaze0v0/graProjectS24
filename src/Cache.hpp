...
public:
    ...
    void process() {
        while (true) {
            ...
            wait(trg.posedge_event());  // Waiting for triggering the signal; 等待触发信号
...
/*                 std::cout << "Index for this round: " << index << std::endl;
                ...
                if (line->valid.read() && line->tag.read() == tag) {
                    ...
                    hit.write(false);
                    if (!we.read()) {                                    // read miss
                        next_addr.write(addr.read());
                        next_we.write(false);  // Read operation from L2
                        std::cout << "Read miss. Proceed to next Structur." << std::endl;
                    } else {                                            // hit miss
                    /* Note: in the case hit miss, we didn't use the [Write allocate] Write-miss policy, that 
                             write the hole cacheline back to the cache with this index / address. 
                             We selected to use the [write around] policy, that only update the cache when hitted.
                             Therefore is a continued write process always "Hit Miss"                   */
                        next_addr.write(addr.read());
                        next_data_out.write(data_in.read());
                        next_we.write(true);  // Write-through to L2
            ...
            done_event.notify();  // Notify: event is done; 通知完成事件
        }
    }
